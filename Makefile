...
#COMPILER_PATH ?= C:\Users\Oli\MinGW\bin
...
#CFLAGS += -Wall -D_DEFAULT_SOURCE -Wno-missing-braces -Wl,--stack,8194
CFLAGS += -Wall -Wno-missing-braces -Wno-unused-variable -Wno-error=implicit-function-declaration -Wl,--stack,8194
#-Wl,--stack,8194
...
#        CFLAGS += $(RAYLIB_PATH)/raylib/raylib.rc.data -Wl,--subsystem,windows
#        CFLAGS += $(RAYLIB_PATH)/raylib/raylib.rc.data
    endif
    ifeq ($(PLATFORM_OS),LINUX)
...
ifeq ($(PLATFORM),PLATFORM_DESKTOP)
    ifeq ($(PLATFORM_OS),BSD)
        ...
        LDFLAGS += -L. -Lraylib -L/usr/local/lib
    endif
    ifeq ($(PLATFORM_OS),LINUX)
        # Reset everything.
        # Precedence: immediately local, installed version, raysan5 provided libs
        LDFLAGS = -L. -L$(RAYLIB_INSTALL_PATH) -L$(RAYLIB_RELEASE_PATH) -L./$(RAYLIB_RELEASE_PATH)/buildc/raylib
    endif
endif
...
# look for all files with corresponding pattern recursively
SRC = $(wildcard $(SRC_DIR)/*.c)
# look for all files with corresponding pattern recursively
SRC_CXX = $(wildcard $(SRC_DIR)/api/*.cpp)

SRC_DIR_CXX = $(SRC_DIR)/api

# name object files with their corresponding source file name.
OBJS_C := $(SRC:%.c=%.o)
...
# first compile the adjacent cpp sources.
$(OBJS_CXX) : $(SRC_CXX)
	$(CXX) $(INCLUDE_PATHS) $(CXXFLAGS) -v -std=c++98 -o $@ -c $<

# TODO: need to add linux, mac, etc support.
run:
#ifeq ($(PLATFORM),PLATFORM_DESKTOP)
#    ifeq ($(PLATFORM_OS),WINDOWS)
	cp $(RES) $(BIN_DIR) && cd $(BIN_DIR) && $(PROJECT_NAME).exe
#	endif
#endif
